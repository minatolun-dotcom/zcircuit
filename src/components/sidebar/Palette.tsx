...
export function Palette() {
  return (
    <aside
      className="flex w-60 shrink-0 flex-col overflow-y-auto border-r border-slate-200 bg-white"
      data-testid="component-palette"
    >
      <div className="border-b border-slate-200 px-3 py-2">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-slate-500">Components</h2>
      ...
      {paletteGroups().map(({ category, items }) => (
        <section key={category} className="px-3 py-2">
          <h3 className="mb-1.5 text-[11px] font-semibold text-slate-400">{CATEGORY_LABEL[category]}</h3>
            ...
            {items.map((meta) => (
              <li key={meta.type}>
                <div
                  ...
                  title={meta.blurb}
                  className="flex cursor-grab items-center gap-2 rounded-md border border-slate-200 px-1.5 py-1 text-slate-700 hover:border-sky-300 hover:bg-sky-50 active:cursor-grabbing"
                >
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="block truncate text-[10px] text-slate-400">{meta.blurb}</span>
                  </span>
                </div>
              </li>
            ))}
